...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  return (
    <main className="bg-zinc-950 min-h-[100dvh] text-zinc-50 relative">
      <Navigation />

      <article className="pt-28 sm:pt-32 pb-24 relative overflow-hidden">
        {/* Background glow graphics */}
        <div className="absolute inset-0 z-0 pointer-events-none">
          <div className="absolute top-0 right-1/4 w-[600px] h-[600px] bg-emerald-500/5 rounded-full filter blur-[140px]"></div>
          <div className="absolute bottom-1/3 left-10 w-[500px] h-[500px] bg-zinc-800/20 rounded-full filter blur-[140px]"></div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8 pt-2">
            <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs sm:text-sm text-zinc-500">
              <Link href="/" className="hover:text-emerald-400 transition-colors">Domov</Link>
              <span>/</span>
              <Link href="/#portfolio" className="hover:text-emerald-400 transition-colors">Naše projekty</Link>
              <span>/</span>
              <span className="text-zinc-100 font-medium">{project.title}</span>
            </nav>
          ...
          <div className="mb-12 max-w-4xl">
            ...
            <p className="text-xl sm:text-2xl text-emerald-400 font-display font-bold mb-4">
              {project.subtitle}
            </p>
              ...
              <div className="bg-zinc-900/70 backdrop-blur-sm border border-zinc-800 rounded-3xl p-8 sm:p-10 shadow-sm">
                ...
                <ul className="space-y-4">
                  {project.features.map((feature, index) => (
                    <li key={`feat-${index}`} className="flex items-start gap-3 text-zinc-300">
                      <span className="w-6 h-6 rounded-full bg-emerald-950/50 text-emerald-400 font-bold flex items-center justify-center shrink-0 mt-0.5 text-xs border border-emerald-800/50">
                        ✓
                      </span>
              ...
              <div className="bg-zinc-900/70 backdrop-blur-sm border border-zinc-800 rounded-3xl p-8 sm:p-10 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {project.deliverables.map((item, index) => (
                    <div key={`deliv-${index}`} className="flex items-center gap-2.5 p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/80 text-sm text-zinc-300 font-medium">
            ...
            <div className="space-y-8 sticky top-28">
              ...
              <div className="bg-zinc-900/70 backdrop-blur-sm border border-zinc-800 rounded-3xl p-6 sm:p-8 shadow-sm">
                ...
                <div className="space-y-3">
                  ...
                  <Link
                    href="/#contact"
                    className="w-full py-3 px-4 bg-zinc-800/80 hover:bg-zinc-700/80 border border-zinc-700/50 hover:border-zinc-600 text-zinc-100 rounded-xl font-bold font-display text-xs sm:text-sm transition-all text-center block"
                  >
                    Kontaktný formulár
                  </Link>
                </div>
          ...
          <div className="border-t border-zinc-800 pt-16">
            <h2 className="text-2xl font-bold font-display text-zinc-50 mb-8">
              Ďalšie projekty
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {otherProjects.map((other) => (
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-emerald-400 inline-flex items-center gap-1 mt-auto">
