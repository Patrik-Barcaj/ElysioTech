                ...
                <div className="flex flex-col md:flex-row items-center gap-12 lg:gap-20"> {/* asymmetric split */}
                    ...
                    <div className="flex-1 text-center md:text-left mt-16 md:mt-0"> {/* text content wrapper */}
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-aurora-purple/10 border border-aurora-purple/30 text-xs font-display tracking-widest text-aurora-purple mb-8"> {/* pre-heading badge */}
                            <span className="w-2 h-2 rounded-full bg-aurora-purple animate-pulse shadow-[0_0_10px_rgba(191,90,242,0.85)] dark:shadow-[0_0_10px_rgba(191,90,242,0.8)]"></span> {/* glowing dot */}
                            DIGITAL AGENCY
                        </div> {/* pre-heading end */}
                        
                        <h1 className="text-5xl md:text-6xl lg:text-7xl font-display font-bold text-gray-900 dark:text-white tracking-tight leading-[1.1] mb-6"> {/* main headline */}
                            <strong>Digitálne riešenia</strong> <br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-aurora-purple via-aurora-indigo dark:via-aurora-green to-gray-800 dark:to-white">pre váš biznis.</span> <br />
                            <span className="text-3xl md:text-4xl text-gray-800 dark:text-gray-300 font-normal">Bez vstupných nákladov a zbytočných zmlúv.</span>
                        </h1> {/* main headline end */}
                        
                        <ul className="text-xl md:text-2xl text-gray-700 dark:text-gray-400 font-light mb-8 max-w-2xl leading-relaxed space-y-4 text-left mx-auto md:mx-0"> {/* subheadline */}
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start"> {/* cta group */}
                            <Link href="#services" className="px-8 py-4 bg-white text-black rounded-xl font-bold font-display hover:bg-gray-200 transition-colors shadow-[0_0_30px_rgba(191,90,242,0.75)] dark:shadow-[0_0_30px_rgba(191,90,242,0.3)] hover:shadow-[0_0_40px_rgba(191,90,242,0.85)] dark:hover:shadow-[0_0_40px_rgba(191,90,242,0.5)] flex items-center justify-center gap-2"> {/* primary cta */}
                            ...
                            <Link href="/cennik" className="px-8 py-4 bg-transparent border border-aurora-green/30 text-gray-900 dark:text-white hover:border-aurora-green/60 rounded-xl font-bold font-display hover:bg-aurora-green/10 transition-all flex items-center justify-center"> {/* secondary cta */}
                    ...
                    <div className="flex-1 relative w-full max-w-lg hidden md:block mt-10 md:mt-0"> {/* image collage wrapper */}
                        <div className="relative w-full aspect-square"> {/* aspect ratio box */}
                            {/* Photo 2: Creative & Production */}
                            <div className="absolute top-0 right-0 w-[80%] h-[75%] border border-aurora-purple/30 rounded-3xl transform rotate-3 hover:rotate-6 hover:scale-105 transition-all duration-700 bg-gray-200 dark:bg-black overflow-hidden shadow-[0_0_40px_rgba(191,90,242,0.38)] dark:shadow-[0_0_40px_rgba(191,90,242,0.15)] z-10 hover:z-30 group cursor-pointer"> {/* background card rotated */}
                                <Image src="/drone-bg.png" alt="Kreatíva a Dizajn" fill className="object-cover opacity-60 group-hover:opacity-90 transition-opacity duration-500" /> {/* drone image */}
                                ...
                                <div className="absolute bottom-6 left-6 right-6 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500"> {/* text content */}
                                    <h3 className="text-2xl font-display font-bold text-gray-900 dark:text-white mb-1 drop-shadow-lg">Kreatíva & <span className="text-aurora-green">Dizajn</span></h3> {/* title */}
                                    <p className="text-sm text-gray-800 dark:text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100 drop-shadow-md">Letecké zábery, grafika a tlač</p> {/* description */}
                                </div> {/* text content end */}
                            </div> {/* background card rotated end */}

                            {/* Photo 1: Tech & Dev */}
                            <div className="absolute bottom-0 left-0 w-[80%] h-[75%] border border-aurora-green/30 rounded-3xl transform -rotate-3 hover:-rotate-6 hover:scale-105 transition-all duration-700 bg-gray-200 dark:bg-black overflow-hidden shadow-[0_0_50px_rgba(161,255,206,0.38)] dark:shadow-[0_0_50px_rgba(161,255,206,0.15)] z-20 hover:z-30 group cursor-pointer"> {/* foreground card rotated */}
                                <Image src="/digital-bg.png" alt="Vývoj a Technológie" fill className="object-cover opacity-60 group-hover:opacity-90 transition-opacity duration-500" /> {/* digital image */}
                                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-90 group-hover:opacity-60 transition-opacity duration-500 pointer-events-none"></div> {/* gradient overlay */}
                                <div className="absolute bottom-6 left-6 right-6 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500"> {/* text content */}
                                    <h3 className="text-2xl font-display font-bold text-gray-900 dark:text-white mb-1 drop-shadow-lg">Vývoj & <span className="text-aurora-purple">Technológie</span></h3> {/* title */}
                                    <p className="text-sm text-gray-800 dark:text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100 drop-shadow-md">Weby, systémy a aplikácie</p> {/* description */}
                                </div> {/* text content end */}
