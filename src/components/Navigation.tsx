...
export default function Navigation() { // export default Navigation block
    ...
    return ( // start return block
        <header className="fixed top-0 w-full z-50 bg-white/80 dark:bg-aurora-dark/80 backdrop-blur-md border-b border-gray-200 dark:border-white/10 transition-all duration-300"> {/* header element */}
                    ...
                    <div className="flex-shrink-0"> {/* logo flex item */}
                        <Link href="/" className="flex items-center gap-2 group"> {/* logo link */}
                            <svg className="h-8 w-auto text-gray-900 dark:text-white group-hover:scale-105 transition-transform duration-300" viewBox="0 0 150 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M15 10L5 30H15L25 10H15Z" fill="currentColor" fillOpacity="0.1"/>
                    ...
                    <div className="hidden md:flex items-center"> {/* desktop menu wrapping */}
                        <div className="ml-10 flex items-baseline space-x-8 font-display"> {/* nav items row */}
                            {navLinks.map((link) => { // map nav links
                                ...
                                return ( // return link
                                    <Link key={link.name} href={link.href} className={`px-3 py-2 rounded-md text-sm font-medium transition-all duration-300 relative ${isActive ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white'}`}> {/* link item */}
                                        {link.name} {/* item text */}
                                        {isActive && <span className="absolute -bottom-1 left-3 right-3 h-0.5 bg-aurora-purple rounded-full shadow-[0_0_8px_rgba(191,90,242,0.85)] dark:shadow-[0_0_8px_rgba(191,90,242,0.6)]"></span>} {/* active indicator */}
                                    </Link> // end link
                                ); // end return
                            })} {/* end map */}
                        </div> {/* nav row */}
                        <div className="ml-4 pl-4 border-l border-gray-200 dark:border-white/10 flex items-center"> {/* Theme toggle wrapper */}
                            <ThemeToggle /> {/* render toggle */}
                        </div> {/* end Theme toggle wrapper */}
                    </div> {/* desktop nav */}
            ...
            {isOpen && ( // open conditional for mobile drop
                <div className="md:hidden bg-black/95 dark:bg-white/95 dark:bg-aurora-dark/95 border-b border-gray-200 dark:border-white/10"> {/* wrapper for mobile panel */}
                    <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 font-display text-center"> {/* inside mobile container */}
                        {navLinks.map((link) => { // map mobile links
                            ...
                            return ( // return link
                                <Link key={link.name} href={link.href} onClick={() => setIsOpen(false)} className={`block px-3 py-4 text-base font-medium transition-colors ${isActive ? 'text-gray-900 dark:text-white bg-black/5 dark:bg-white/5 border-l-4 border-aurora-purple' : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 border-l-4 border-transparent'}`}> {/* link item */}
                                    {link.name} {/* item text */}
                                </Link> // end link
                            ); // end return
                        })} {/* end map */}
