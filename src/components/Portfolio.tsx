...
import echoLogo from '../assets/websites/Echo_logo.png'; // import echo logo
...
export default function Portfolio() { // export Portfolio component function
    const projects = [ // array of web/app projects
        ...
        { title: 'Echo', category: 'Fyzická Zbierka & Spotify', src: echoLogo.src, link: 'https://echo.elysiotech.com', bgClass: 'bg-gray-200 dark:bg-black' }, // project 3
    ...
    return ( // return JSX layout
        <section id="portfolio" className="py-24 bg-gray-100 dark:bg-[#0a0715] relative border-t border-black/5 dark:border-white/5"> {/* section wrapper */}
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10"> {/* container block */}
                <div className="text-center mb-16"> {/* header block */}
                    ...
                    <h3 className="text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white font-display">Naše Portfólio</h3> {/* title */}
                </div> {/* header block end */}

                {/* Unified Portfolio */}
                <div className="mb-20"> {/* portfolio wrapper */}

                    <h4 className="text-2xl font-bold font-display text-gray-900 dark:text-white mb-8 border-l-4 border-aurora-purple pl-4">Naše <span className="text-aurora-purple">Projekty</span></h4> {/* projects title */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16"> {/* projects layout */}
                        {projects.map((proj, idx) => ( // map projects
                            <a key={idx} href={proj.link} target={proj.link !== '#' ? "_blank" : undefined} rel={proj.link !== '#' ? "noopener noreferrer" : undefined} className={`group relative aspect-square overflow-hidden rounded-2xl ${proj.bgClass || 'bg-black/5 dark:bg-white/5'} border border-black/10 dark:border-white/10 cursor-pointer block`}> {/* project card */}
                                <img src={proj.src} alt={proj.title} className="object-cover w-full h-full transform transition-transform duration-700 group-hover:scale-110 opacity-100 dark:opacity-70 group-hover:opacity-100" /> {/* project image */}
                                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/80 to-black/20 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end"> {/* overlay */}
                                    <div className="p-5 w-full"> {/* padding wrapper */}
                                        ...
                                        {proj.link !== '#' && <span className="text-xs text-white/70 mt-1 block">Zobraziť web ↗</span>} {/* link hint */}
                                    </div> {/* padding end */}
                                </div> {/* overlay end */}
                            </a> // project card end
                        ))} {/* map projects end */}
                    ...
                    <h4 className="text-2xl font-bold font-display text-gray-900 dark:text-white mb-8 border-l-4 border-aurora-green pl-4">Foto & Video <span className="text-aurora-green">Galéria</span></h4> {/* gallery title */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6"> {/* gallery layout */}
                        {gallery.map((gal, idx) => ( // map gallery
                            <div key={idx} onClick={() => openLightbox(idx)} className="group relative aspect-square overflow-hidden rounded-2xl bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 cursor-pointer"> {/* gallery card */}
                                <img src={gal.src} alt={gal.title} className="object-cover w-full h-full transform transition-transform duration-700 group-hover:scale-110 opacity-100 dark:opacity-70 group-hover:opacity-100" /> {/* gallery image */}
                ...
                <div className="mt-16 text-center"> {/* bottom button container */}
                    <button onClick={() => alert('Všetky naše projekty momentálne nahrávame do galérie. Ďalšie referencie čoskoro pribudnú!')} className="px-8 py-4 bg-transparent border border-aurora-purple text-gray-900 dark:text-white rounded-full font-bold hover:bg-aurora-purple/20 transition-all duration-300"> {/* display alert on click */}
            ...
            {lightbox !== null && ( // lightbox conditional render
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-200 dark:bg-black/95 p-4" onClick={closeLightbox}> {/* lightbox overlay */}
                    <button onClick={closeLightbox} className="absolute top-6 right-6 text-gray-900 dark:text-white/70 hover:text-gray-900 dark:text-white transition-colors z-50"> {/* close button */}
                    ...
                    <button onClick={prevImage} className="absolute left-4 sm:left-10 text-gray-900 dark:text-white/70 hover:text-gray-900 dark:text-white transition-colors z-50 bg-gray-200 dark:bg-black/50 p-3 rounded-full"> {/* prev button */}
                    ...
                    <button onClick={nextImage} className="absolute right-4 sm:right-10 text-gray-900 dark:text-white/70 hover:text-gray-900 dark:text-white transition-colors z-50 bg-gray-200 dark:bg-black/50 p-3 rounded-full"> {/* next button */}
                    ...
                    <div className="absolute bottom-6 left-0 right-0 text-center text-gray-900 dark:text-white font-bold text-lg drop-shadow-lg pointer-events-none"> {/* title wrapper */}
