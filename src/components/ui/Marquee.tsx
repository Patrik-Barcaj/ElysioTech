...
export default function Marquee({ items, speed = 40 }: MarqueeProps) { // export Marquee component
    return ( // return JSX layout
        <div className="relative w-full overflow-hidden bg-[#080808] border-y border-neutral-800 py-6 flex items-center"> {/* marquee wrapper */}
            {/* Fade edges */}
            <div className="absolute inset-y-0 left-0 w-32 bg-gradient-to-r from-[#080808] to-transparent z-10"></div> {/* left fade */}
            <div className="absolute inset-y-0 right-0 w-32 bg-gradient-to-l from-[#080808] to-transparent z-10"></div> {/* right fade */}
            ...
            > {/* track content */}
                ...
                {[...items, ...items, ...items, ...items].map((item, index) => ( // map duplicated items
                    <div key={index} className="flex items-center px-8"> {/* item wrapper */}
                        <span className="text-2xl font-bold font-display text-white/40 uppercase tracking-widest hover:text-[#00D26A] transition-colors duration-300"> {/* item text */}
                            {item} {/* render text */}
                        </span> {/* text end */}
                        <span className="w-2 h-2 rounded-full bg-[#FFB800] mx-8"></span> {/* separator dot */}
